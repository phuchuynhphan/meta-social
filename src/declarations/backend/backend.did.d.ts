...
export type ResultFriends = { ok: Array<Friend> } | { err: Error };
export type ResultMessages =
  | {
      ok: { status: ChatStatus; messages: Array<Message__1> };
    }
  | { err: Error };
export type ResultNat = { ok: bigint } | { err: Error };
export type ResultQuestion = { ok: Question } | { err: Error };
export type ResultQuestionStats = { ok: Array<QuestionStats> } | { err: Error };
export type ResultSkip = { ok: Skip } | { err: Error };
export type ResultTeam = { ok: TeamInfo } | { err: Error };
export type ResultTeamStats = { ok: TeamStats } | { err: Error };
export type ResultTeams = { ok: Array<TeamUserInfo> } | { err: Error };
export type ResultUser = { ok: UserPermissions } | { err: Error };
export type ResultUserMatches = { ok: Array<UserMatch> } | { err: Error };
export type ResultUserPermissions =
  | { ok: Array<UserPermissions> }
  | { err: Error };
export type ResultUsers = { ok: Array<User> } | { err: Error };
export type ResultUsersNotifications =
  | { ok: Array<UserNotifications> }
  | { err: Error };
export type ResultVoid = { ok: null } | { err: Error };
...
export interface _SERVICE {
  ...
  >;
  backupQuestions: ActorMethod<[string, bigint, bigint], Array<StableQuestion>>;
