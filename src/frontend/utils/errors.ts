...
const notifyWith = <A, R>(
  effect: Effect.Effect<A, FrontendError, R>,
  msg?: (a: A) => string,
): Effect.Effect<A, FrontendError, R> => {
  return Effect.mapBoth(effect, {
    ...
    onSuccess: (a) => {
      if (msg) addNotification("ok", msg(a));
      return a;
    },
  });
};
...
const notifyMsg = <A, R>(
  ...
  return Effect.tapBoth(effect, {
    ...
    onSuccess: (a) => {
      if (msg) addNotification("ok", msg);
      return Effect.succeed(a);
    },
  });
};

export const notifyWithMsg = <A>(
  msg?: string,
): ((effect: FrontendEffect<A>) => FrontendEffect<A>) => {
  return (effect) => notifyMsg(effect, msg);
};

export const notifyWithFormatter = <A>(
  msg: (a: A) => string,
): ((effect: FrontendEffect<A>) => FrontendEffect<A>) => {
...
export const is = (
  ...
  switch (err.tag) {
    case "backend":
      return getErrorKey(err.err) === key;
    case "form":
      return getErrorKey(err.err) === key;
    case "network":
      return true;
