...
const networkDataToPromise = <T>(data: NetworkData<T>): Promise<T> => {
  ...
  if (data.status === "requested" && data.lastOK)
...
type FrontendEffectToEffect<A> = (
  effect: FrontendEffect<A>,
) => FrontendEffect<A>;

const setRequested = <A>(
  old: NetworkData<A>,
  reset: boolean,
): NetworkData<A> => {
  return {
...
export const storeToData = <A>(
  old: NetworkData<A>,
  effect: FrontendEffect<A>,
  store: (a: NetworkData<A>) => void,
  ...
  return pipe(before, () => effect, after);
};

export const runStoreNotify = <A>(
  old: NetworkData<A>,
  effect: FrontendEffect<A>,
  store: (a: NetworkData<A>) => void,
  msg?: string,
  ...
  return Effect.runPromise(
    storeToData(old, pipe(effect, notifyWithMsg(msg)), store),
  );
};

export const runStore = <A>(
  old: NetworkData<A>,
  effect: FrontendEffect<A>,
  store: (a: NetworkData<A>) => void,
  silent: boolean = false,
): Promise<A> => {
  return Effect.runPromise(storeToData(old, effect, store, silent));
};

export const runNotify = <A>(
  effect: FrontendEffect<A>,
  msg?: string,
): Promise<A> => {
...
const combineNetworkData = <A>(
  ...
  switch (newer.status) {
    ...
    case "error":
    case "requested":
      return { ...newer, data: old.data, lastOK: old.lastOK };
    case "init":
      throw "combineNetworkData should never be called with init as new value";
  }
};
...
interface DataBy<T> {
  get: (group: string) => Ref<NetworkData<T>>;
}
...
export const getAppState = () => {
  return {
    ...
    teams: mk<TeamUserInfo[]>(appData.teams, () =>
      backend.loadTeams(appData.knowTeams),
    ),
    friends: mk<Friend[]>(appData.friends, () =>
      backend.loadFriends(appData.team),
    ),
    chat: mkBy1<{ messages: Message[]; status: ChatStatus }, string>(
      appData.chat,
      (user: string) => (team: string) => backend.getMessages(team, user),
    ),
    ...
    getTeam(orRedirect: boolean = true): TeamUserInfo | null {
      let t = null;
      this.setTeam(
        inBrowser() ? window.localStorage.getItem("team") || "" : "",
      );
      if (appData.teams.value.status === "ok") {
        t =
          appData.teams.value.data?.find((t) => t.key === appData.team) || null;
        if (!orRedirect) {
          // don't redirect
        } else if (!t) {
          navTo("/select-team");
    ...
    joinTeam(code: string, invitedBy: string | null): Promise<void> {
      return runNotify(
        backend.joinTeam(appData.team, code, invitedBy),
        "Welcome to the team!",
      );
    },
    ...
    createTeam(
      ...
      return runNotify(
        backend.createTeam(team, name, about, logo, listed, code),
        "Welcome to the team!",
      );
    },
    updateTeam(
      ...
      return runNotify(
        backend.updateTeam(team, name, about, logo, listed, code),
        "Welcome to the team!",
      );
    },
    deleteQuestion(q: Question, hide: boolean) {
      return runNotify(
        backend.deleteQuestion(appData.team, q, hide),
        hide ? "Question removed" : "Question recovered",
      );
    },
    deleteAnswers(confirm: string) {
      return runNotify(
        backend.deleteAnswers(appData.team, confirm),
        "Answers removed",
      );
    },
...
const mk = <T>(
  store: Ref<NetworkData<T>>,
  action: () => FrontendEffect<T>,
  empty: T | null = null,
) => {
...
const mk2 = <T, P1, P2>(
  store: Ref<NetworkData<T>>,
  action: (p1: P1, p2: P2) => FrontendEffect<T>,
  empty: T | null = null,
) => {
  const get = (): NetworkData<T> => store.value;
  const set = (next: NetworkData<T>): void => {
...
const mkBy1 = <T, P1>(
  store: DataBy<T>,
  action: (a: string) => (p1: P1) => FrontendEffect<T>,
  empty: T | null = null,
) => {
  const x: any = null;
...
const mkBy2 = <T, P1, P2>(
  store: DataBy<T>,
  action: (a: string) => (p1: P1, p2: P2) => FrontendEffect<T>,
  empty: T | null = null,
) => {
  const x: any = null;
...
export const useAppState = defineStore({
  ...
  actions: {
    ...
    setOpenQuestions(
      qs: NetworkData<Question[]>,
      noAnswered: boolean = true,
    ): void {
    ...
    removeOpenQuestion(q: Question): void {
      ...
      if (this.answeredIdsReset) {
        clearTimeout(this.answeredIdsReset);
      }
      this.answeredIdsReset = setTimeout(() => {
        this.answeredIds = [];
      }, 10_000);
      if (data.data) {
    ...
    loadOpenQuestions(maxAgeS?: number, msg?: string): Promise<Question[]> {
      if (shouldUpdate(this.openQuestions, maxAgeS)) {
        ...
        return runStoreNotify(
          old,
          backend.loadOpenQuestions(this.team),
          this.setOpenQuestions,
          msg,
        );
      } else {
    ...
    loadAnsweredQuestions(maxAgeS?: number): void {
      if (shouldUpdate(this.answeredQuestions, maxAgeS)) {
        ...
        runStoreNotify(
          old,
          backend.getAnsweredQuestions(this.team),
    ...
    loadOwnQuestions(maxAgeS?: number): void {
      if (shouldUpdate(this.ownQuestions, maxAgeS)) {
        ...
        runStoreNotify(
          old,
          backend.getOwnQuestions(this.team),
          this.setOwnQuestions,
        );
      }
    },
    ...
    loadUser(
      ...
      if (shouldUpdate(old, maxAgeS)) {
        return runStore(
          old,
          backend.loadUser(false).pipe(
            Effect.mapError((err) => {
              if (!orRedirect) return err;
              if (errors.is(err, "backend", "notRegistered")) {
                console.log(
                  "redirect to /register because user is not registered",
                  err,
                );
                navTo("/register");
    ...
    sendFriendRequest(username: string): Promise<void> {
      return runNotify(
        backend.sendFriendRequest(this.team, username),
        "Friend request send",
      );
    },
    answerFriendRequest(username: string, accept: boolean): Promise<void> {
      return runNotify(
        backend.answerFriendRequest(this.team, username, accept),
    ...
    loadTeams(maxAgeS?: number, known?: string): Promise<TeamUserInfo[]> {
      ...
      if (shouldUpdate(this.teams, maxAgeS)) {
        return runStore(
          this.teams,
          backend.loadTeams(this.knownTeams),
          this.setTeams,
        );
      } else {
    ...
    joinTeam(code: string, invitedBy: string | null): Promise<void> {
      return runNotify(
        backend.joinTeam(this.team, code, invitedBy),
        "Welcome to the team!",
      );
    },
    leaveTeam(user: string): Promise<void> {
    ...
    deleteQuestion(q: Question, hide: boolean) {
      return runNotify(
        backend.deleteQuestion(this.team, q, hide),
        hide ? "Question removed" : "Question recovered",
      );
    },
    deleteAnswers(confirm: string) {
      return runNotify(
        backend.deleteAnswers(this.team, confirm),
        "Answers removed",
      );
    },
    deleteUser(confirm: string) {
    ...
    loadTeamStats(maxAgeS?: number) {
      ...
      if (shouldUpdate(old, maxAgeS)) {
        runStore(
          old,
          backend.loadTeamStats(this.team),
          this.setTeamStats,
        ).catch(console.error);
      }
    },
    ...
    loadQuestionStats(maxAgeS?: number) {
      ...
      if (shouldUpdate(old, maxAgeS)) {
        runStore(
          old,
          backend.loadQuestionStats(this.team),
    ...
    loadTeamMembers(maxAgeS?: number) {
      if (shouldUpdate(this.teamMembers, maxAgeS)) {
        runStore(
          this.teamMembers,
          backend.loadTeamMembers(this.team),
          this.setTeamMembers,
        ).catch(console.error);
      }
    },
    ...
    loadTeamAdmins(maxAgeS?: number) {
      ...
      if (shouldUpdate(current, maxAgeS)) {
        runStore(
          current,
          backend.loadTeamAdmins(this.team),
          this.setTeamAdmins,
        ).catch(console.error);
      }
    },
    ...
    loadAdmins(maxAgeS?: number) {
      if (shouldUpdate(this.getAdmins(), maxAgeS)) {
        runStore(this.getAdmins(), backend.loadAdmins(), this.setAdmins).catch(
          console.error,
        );
      }
    },
    ...
    loadUsers(maxAgeS?: number) {
      if (shouldUpdate(this.users, maxAgeS)) {
        runStore(this.users, backend.loadUsers(), this.setUsers).catch(
          console.error,
        );
      }
    },
...
export const addNotification = (
  ...
  switch (level) {
    case "ok":
      ...
      break;
    case "warning":
      console.warn("notification (" + level + "): " + msg);
      break;
    case "error":
      console.warn("notification (" + level + "): " + msg);
      break;
...
export const invitePath = (
  ...
  const params = user
    ? new URLSearchParams({ invite, by: user })
    : new URLSearchParams({ invite });
